...
def deploy() -> None:
    """Deploy the Simple AMM contract"""
    # Get network client
    ...
    artifacts_path = Path(__file__).parent / ".." / "artifacts" / "simple_amm"
    
    # Read approval program
    with open(artifacts_path / "SimpleAMM.approval.teal", 'r') as f:
        approval_teal = f.read()
    
    # Read clear program  
    with open(artifacts_path / "SimpleAMM.clear.teal", 'r') as f:
        clear_teal = f.read()
