...
def deploy() -> None:
    """Deploy the Yield Tokenization contract"""
    # Get network client
    ...
    artifacts_path = Path(__file__).parent / ".." / "artifacts" / "yield_tokenization"
    
    # Read approval program
    with open(artifacts_path / "YieldTokenization.approval.teal", 'r') as f:
        approval_teal = f.read()
    
    # Read clear program  
    with open(artifacts_path / "YieldTokenization.clear.teal", 'r') as f:
        clear_teal = f.read()
    ...
    print(f"Yield Tokenization contract deployed successfully! App ID: {app_id}")
